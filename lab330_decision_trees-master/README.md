...
* __-a__ _folder_ or __--algorithm__ _folder_  
   The name of the algorithm (the folder where the MLAlgorithm.py is stored).
   __REQUIRED__

* __-d__ _file_path_ or __--dataFile__ _file_path_  
   A file path to a CSV data file.  __REQUIRED__

* __-f__ _column_numbers_ or __--fields__ _column_numbers_  
...
* __-p__ _key_value_pairs_ or __--parameters__ _key_value_pairs_  
...
* __-s__ _float_ or __--splitRatio__ _float_  
   The split ratio will default to *0.7* unless provided.

**Examples:**

```sh
    python3 run_ml_algorithm.py -a example -d sample.csv -f "8" -p "foo:bar|numbers:1,2,3"
    python3 run_ml_algorithm.py -a knn -d sample.csv -f "8" -p "k:4"
    python3 run_ml_algorithm.py -a naive_bayes -d sample.csv -f "8"
    python3 run_ml_algorithm.py -a neural_network -d sample.csv -f "8" -p "inputSize:8|outputSize:1|hiddenLayers:8"
```